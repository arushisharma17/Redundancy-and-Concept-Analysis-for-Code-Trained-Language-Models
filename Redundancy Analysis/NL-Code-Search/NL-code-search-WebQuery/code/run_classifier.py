...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in train_iterator:
        ...
        for step, batch in bar:
            ...
            loss, predictions,hidden_states = model(code_inputs, nl_inputs, labels)
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        labels = batch[2].to(args.device)
        with torch.no_grad():
            lm_loss, predictions,hidden_states = model(code_inputs, nl_inputs, labels)
...
def main():
    ...
    config.num_labels = 2
    #config.eos_token_id=50257 #non of these are getting updated 
    #config.pad_token_id =50258
    print("config",config)
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.encoder_name_or_path,
                                                padding=True, do_lower_case=args.do_lower_case,
                                                cache_dir=args.cache_dir if args.cache_dir else None)
    print("old vocab size", tokenizer.vocab_size)

    #tokenizer.add_special_tokens({'pad_token': '<pad>'})#these are not working nothing is being added.
    #tokenizer.pad_token_id=config.pad_token_id
    #tokenizer.bos_token = '<s>'
    #tokenizer.eos_token = '</s>'
    #tokenizer.vocab_size = len(tokenizer)

    print("new tokenizer vocab size", tokenizer.vocab_size)#50257
    ...
    print("length of tokenizer vocabulary",len(tokenizer))#50258
    if tokenizer.pad_token is None:
        tokenizer.add_special_tokens({'pad_token': '<pad>'})
        model.resize_token_embeddings(len(tokenizer))
        print("pad_token_id after adding", tokenizer.pad_token_id, config.pad_token_id)

    #irint("pad_token_id", config.pad_token_id, tokenizer.pad_token_id, tokenizer.pad_token,config.pad_token)
    # model.resize_token_embeddings(len(tokenizer))    
