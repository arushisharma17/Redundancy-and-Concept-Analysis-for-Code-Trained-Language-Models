...
def convert_examples_to_features(js, tokenizer, args):
    ...
    print("tokenizer.pad_token_id code utils",tokenizer.pad_token_id)
    code_ids += [tokenizer.pad_token_id]*padding_length

    nl = js['doc']  # query
    nl_tokens = tokenizer.tokenize(nl)[:args.max_seq_length-2]
    if args.model_type=="gpt2":
        nl_tokens = [tokenizer.bos_token]+nl_tokens+[tokenizer.eos_token]
    else:
        nl_tokens = [tokenizer.cls_token]+nl_tokens+[tokenizer.sep_token]
    
    nl_ids = tokenizer.convert_tokens_to_ids(nl_tokens)
    padding_length = args.max_seq_length - len(nl_ids)
    print("tokenizer.pad_token_id nl utils",tokenizer.pad_token_id)
